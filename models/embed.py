...
class TokenEmbedding(nn.Module):
    def __init__(self, c_in, d_model, tao=2, m=7, pad=True, is_split=False):
    ...
    def forward(self, x):
        ...
        if self.is_split == True:
            ...
            x_embedded1 = torch.split(x_embedded, self.m + 1, dim=7)
            ...
            x_embedded = torch.cat(channel_splitter, dim=7).transpose(1, 2)
...
class DataEmbedding(nn.Module):
    def __init__(self, c_in, d_model, embed_type='fixed', freq='h', dropout=0.1):
        ...
        self.value_embedding = TokenEmbedding(c_in=c_in, d_model=d_model, m=7, tao=2)
